...
using mpBackup.MpUtilities.Http;
...
namespace mpBackup.MpGoogle
{
    ...
    /// Jan 2015 - MP modification to suit mpBackup.
    /// </remarks>
    ...
    /// </typeparam>
    public class MpResumableUpload<TRequest>
    {
        #region Constants
        ...
        /// </remarks>
        protected MpResumableUpload(IClientService service, string path, string httpMethod, Stream contentStream,
            string contentType)
        {
        ...
        {
            private MpResumableUpload<TRequest> Owner { get; set; }
            ...
            /// </summary>
            public ServerErrorCallback(MpResumableUpload<TRequest> resumable)
            {
        ...
        public async Task<IUploadProgress> UploadAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                log.Debug("MediaUpload[" + UploadUri.AbsoluteUri + "] - Start uploading...");
            }
            catch (Exception ex)
            {
                log.Error("MediaUpload - Exception occurred while initializing the upload: ", ex);
        ...
        public async Task<IUploadProgress> ResumeAsync(CancellationToken cancellationToken)
        {
            if (UploadUri == null)
            {
                log.Info("There isn't any upload in progress, so starting to upload again");
            ...
            {
                log.Error("MediaUpload[{0}] - Task was canceled", ex);
                UpdateProgress(new ResumableUploadProgress(ex, BytesServerReceived));
                throw ex;
            }
            catch (Exception ex)
            {
                log.Error("MediaUpload[{0}] - Exception occurred while resuming uploading media", ex);
                UpdateProgress(new ResumableUploadProgress(ex, BytesServerReceived));
                return Progress;
            }
        ...
        private async Task<IUploadProgress> UploadCoreAsync(CancellationToken cancellationToken)
        {
            ...
            {
                log.Error("MediaUpload[{0}] - Exception occurred while uploading media", ex);
                UpdateProgress(new ResumableUploadProgress(ex, BytesServerReceived));
            }
        ...
        protected async Task<bool> SendNextChunkAsync(Stream stream, CancellationToken cancellationToken)
        {
            ...
            //log.Debug("MediaUpload[{0}] - Sending bytes={1}-{2}", UploadUri, BytesServerReceived,
            //    BytesClientSent - 1);
        ...
        private async Task<bool> HandleResponse(HttpResponseMessage response)
        {
            ...
            else if (response.StatusCode == (HttpStatusCode)308)
            {
                ...
                log.Debug("MediaUpload[" + UploadUri + "] - {" + BytesServerReceived + "} Bytes were sent successfully");
                return false;
            }

            var error = await Service.DeserializeError(response).ConfigureAwait(false);
            throw new Google.GoogleApiException(Service.Name, error.ToString())
            {
        ...
        {
            log.Debug("MediaUpload[{0}] - media was uploaded successfully");
        ...
        private HttpRequestMessage CreateInitializeRequest()
        {
            ...
            // init parameters
            builder.AddParameter(Google.Apis.Util.RequestParameterType.Query, "key", Service.ApiKey);
            builder.AddParameter(Google.Apis.Util.RequestParameterType.Query, "uploadType", "resumable");
                ...
                if (attribute != null)
                {
                    ...
                    if (value != null)
                    {
                        ...
                        if (!(value is string) && valueAsEnumerable != null)
                        {
                            ...
                            {
                                requestBuilder.AddParameter(attribute.Type, name, MpUtilities.MpUtilities.ConvertToString(elem));
                            }
                        }
                        else
                        {
                            ...
                            requestBuilder.AddParameter(attribute.Type, name, MpUtilities.MpUtilities.ConvertToString(value));
                        }
                    }
                }
            }
    ...
    /// </typeparam>
    public class ResumableUpload<TRequest, TResponse> : MpResumableUpload<TRequest>
    {
        #region Construction
